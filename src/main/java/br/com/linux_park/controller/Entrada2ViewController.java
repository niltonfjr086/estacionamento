...
public class Entrada2ViewController implements Initializable {
    ...
    private void defineVagasDisponiveis() {
        defineQtdMotos();
        defineQtdCarros();
        defineQtdCaminhonetes();
        defineQtdeCaminhoes();
        defineToleranciaDefault();
    }

    private void defineQtdMotos() {
        lblVagasMoto.setText(ev.tipoVagaDAO.getPorId(ev.tipoVeiculoDAO.get("Moto").getTipoVaga().getId()).getQuantidade().toString());
    }

    private void defineQtdCarros() {
        lblVagasCarro.setText(ev.tipoVagaDAO.getPorId(ev.tipoVeiculoDAO.get("Carro").getTipoVaga().getId()).getQuantidade().toString());
    }

    private void defineQtdCaminhonetes() {
        lblVagasCaminhonete.setText(ev.tipoVagaDAO.getPorId(ev.tipoVeiculoDAO.get("Caminhonete").getTipoVaga().getId()).getQuantidade().toString());
    }

    private void defineQtdeCaminhoes() {
        lblVagasCaminhao.setText(ev.tipoVagaDAO.getPorId(ev.tipoVeiculoDAO.get("Caminhao").getTipoVaga().getId()).getQuantidade().toString());
    }
